...
class tongbutuiSpider(BaseSpider):
    ...
    """
    name = "kuaiyong_app"
    allowed_domains = ['kuaiyong.com/']
    start_urls = [
        'http://app.kuaiyong.com/list/type?device=1&appTag=2',
        ]  
    ...
    def parse_all_game(self, response):
		...
		urls = hxs.select('//a[@class='app_pic fl' and p/@class='shade s64']/@href').extract()
		#self.printhxs(response.meta['game_category'])
		for url in urls[0:]:
			#print url
		...
		urls = hxs.select("//a[@class='next' and (text()=u'下一页')]/@href").extract() or ['none']
		if(urls[0] != 'none'):
			
			request_next_page = Request(urljoin_rfc(base_url, urls[0]), dont_filter = True, callback = self.parse_all_game)
...
#handle the response of new requests in category
    def parse_category(self, response):
		...
		url = urljoin_rfc(base_url, hxs.select('//a[(text()=u'简洁模式')]/@href').extract()[0])
		#print url
    ...
    def parse(self, response):
		...
		urls = hxs.select("//body/div/div/div/ul/a@href").extract()or['']
